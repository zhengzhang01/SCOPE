...
import os
from scope.dataset.transform import Resize, NormalizeImage, PrepareForNet, Crop, ColorJitter, GaussianBlur
import random
from typing import Dict, List, Tuple
import numpy as np

class ParallelDomain4D(Dataset):
    def __init__(self, filelist_path: str, mode: str, size: Tuple[int, int] = (518, 518), cj_p = 0.0, cj_s=1.0, g_p=0.0, g_s=1.0, r_p=0.0, r_a=30):
        """
        Initialize ParallelDomain4D dataset
        Args:
    ...
    def __getitem__(self, item: int) -> Dict:
        ...
        # Read depth from npz file
        depth_data = np.load(depth_path)
        # Most npz files store the main depth array under a specific key.
        depth = depth_data['depth'] if 'depth' in depth_data.keys() else depth_data[depth_data.files[0]]
        
        # Ensure depth is in correct format (float32)
...
class ParallelDomain4DScene(Dataset):
    """Dataset loader for ParallelDomain4D with grouped scenes and interval-based sampling."""
    ...
    def _initialize_scenes(self, filelist_path: str):
        ...
        for line in filelist:
            ...
            # For path like .../scene_000000/rgb/camera0/000000000000000005.png
            path_parts = img_path.split('/')
            scene_folder = path_parts[-4]  # scene_000000
            camera_folder = path_parts[-2]  # camera0
            scene_name = f"{scene_folder}/{camera_folder}"
            
            # Extract frame number (last 5 digits from the filename)
            frame_num = int(path_parts[-1].split('.')[0][-5:])
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        # Load first depth
        depth_data = np.load(group[0][1])
        first_depth = depth_data['depth'] if 'depth' in depth_data.keys() else depth_data[depth_data.files[0]]
        ...
        for img_path, depth_path in group:
            ...
            # Read depth from npz file
            depth_data = np.load(depth_path)
            # Extract depth array from npz file
            depth = depth_data['depth'] if 'depth' in depth_data.keys() else depth_data[depth_data.files[0]]
            depth = depth.astype(np.float32)
