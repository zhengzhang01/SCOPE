...
def load_depth(depth_path, is_float16=True):
    """Load depth from EXR file and handle invalid depth values."""
    image = cv2.imread(depth_path, cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH)[..., 0]  # (H, W)
    if is_float16:
        invalid_mask = (image == 65504)  # EXR invalid value
    else:
        invalid_mask = None
    image = image / 10000.0  # Convert cm to meters
    return image, invalid_mask


class MatrixCityDataset(Dataset):
    """Dataset loader for MatrixCity with grouped scenes and interval-based sampling."""
    ...
    def __init__(
        ...
        current_epoch: int = 0,
        duplicate_times: int = 1,  # Data duplication factor
        disparity: bool = True
    ):
        """
    ...
    def _initialize_scenes(self, filelist_path: str):
        ...
        for line in filelist:
            ...
            # Extract scene name from the path (second-to-last folder is the scene)
            path_parts = img_path.split('/')
            scene_name = path_parts[-2]  # Scene is the second-to-last folder
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        first_depth, first_invalid_mask = load_depth(group[0][1])
        
        # Apply transforms up to crop
        sample = {'image': first_image, 'depth': first_depth, 'mask': first_invalid_mask}
        ...
        for img_path, depth_path in group:
            ...
            # Load and process depth (convert to meters)
            depth, invalid_mask = load_depth(depth_path)
            
            sample = {'image': image, 'depth': depth, 'mask': invalid_mask}
            ...
            transformed_invalid_mask = torch.from_numpy(sample['mask']).bool()

            # Create valid mask (depth > 0 and handle invalid mask)
            valid_mask = (transformed_depth > 0) & (~transformed_invalid_mask) & (transformed_depth <= 80) 
