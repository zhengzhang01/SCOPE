...
class SUNRGBD(Dataset):
    ...
    def __getitem__(self, item):
        ...
        # Read depth map (assuming depth is stored as a grayscale image)
        ...
        sample['image_path'] = img_path.split('SUNRGBD/')[-1]
        sample['depth_path'] = depth_path.split('SUNRGBD/')[-1]
...
class SUNRGBDScene(Dataset):
    def __init__(self, filelist_path, mode, size=(518, 518)):
        ...
        for line in lines:
            ...
            # Extract scene name from the folder under SUNRGBD (e.g., NYUdata)
            # Assuming path contains 'SUNRGBD/{subfolder}/{scene_name}/...'
            parts = img_path.split(os.sep)
            try:
                scene_index = parts.index('SUNRGBD') + 2  # The folder after SUNRGBD/kv1 is the scene name
    ...
    def __getitem__(self, idx):
        ...
        for img_path, depth_path in scene:
            ...
            # Load depth map (assuming depth is stored as a grayscale image)
            depth = np.asarray(Image.open(depth_path), dtype='uint16') / 10000.0
