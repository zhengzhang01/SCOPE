    ...
    def forward(self, x):
        if x.shape[0] > 120:  
            ...
            for i in range(0, x.shape[0], 120):
                end_idx = min(i + 120, x.shape[0])
                chunk = x[i:end_idx]
                
                # Process chunk normally
                skip = self.skip_connection(chunk)
                chunk_out = self.layers(chunk)
                chunk_out = chunk_out + skip
...
class MoGeModel(nn.Module):
        ...
        **extra_kwargs
        ...
        if extra_kwargs:
            if 'trained_area_range' in extra_kwargs:
                num_tokens_range = [extra_kwargs['trained_area_range'][0] // 14 ** 2, extra_kwargs['trained_area_range'][1] // 14 ** 2]
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Union[str, Path, IO[bytes]], model_kwargs: Optional[Dict[str, Any]] = None, **hf_kwargs) -> 'MoGeModel':
        ...
        ### Returns:
        - A new MoGe model instance with the parameters loaded from the checkpoint.
        """
