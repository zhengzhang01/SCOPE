...
def precompute_freqs_cis_with_dynamic_ntk(dim: int, end: int, base: float = 10000.0, 
                                           train_seq_len: int = 24):
    """
    Precompute RoPE frequencies with dynamic NTK scaling
    
    Args:
        dim: Hidden dimension
        end: Current sequence length
        base: RoPE base frequency parameter
        train_seq_len: Sequence length during training
    """
    # Only apply scaling when current sequence length exceeds training length
    scale = end / train_seq_len if end > train_seq_len else 1.0
    
    # Calculate NTK scaling factor: high-frequency components are scaled less, low-frequency components are scaled more
    # This helps maintain better extrapolation performance
    power = torch.arange(0, dim, 2).float() / dim
    scale_factor = scale ** (1.0 - power)
    
    # Apply NTK scaling to frequencies
    freqs = 1.0 / (base ** (torch.arange(0, dim, 2)[: (dim // 2)].float() / dim))
    freqs = freqs * scale_factor[: (dim // 2)]
    
    # Generate position indices and compute outer product
    t = torch.arange(end, device=freqs.device, dtype=torch.float32) # [24]
    freqs = torch.outer(t, freqs) # [24, 512]
    
    # Convert to complex form
    freqs_cis = torch.polar(torch.ones_like(freqs), freqs) # [24, 512]
    return freqs_cis